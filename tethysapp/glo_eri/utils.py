...
def get_counties_options():
    ...
    wfs_request_url = geoserver_wfs_url + '?version=1.0.0&request=GetFeature&' \
                                          'typeNames=glo_eri:TexasCounties&outputFormat=application/json'
...
def get_legend_options():
    ...
    for type, layers in options.items():
        for layer in layers:
            ...
            legend_url = geoserver_wms_url + common_req_str + "&LAYER=glo_eri:" + type + "&STYLE=" + style
